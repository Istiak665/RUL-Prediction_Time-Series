    ...
    while j <= 100:
        ...
        for t in range(x_test.shape[0]): # iterate to the end of each sequence
            if t == 0:
                ...
                # zeros_row = np.zeros(x_test.shape[1])  # Create a row of zeros with the same number of columns
                # X_test = np.append(x_test[t - 1:, 2:], [zeros_row], axis=0)

                X_test = np.append(x_test[t - 1:, 2:], [np.zeros(84)], axis=0)
            else:
